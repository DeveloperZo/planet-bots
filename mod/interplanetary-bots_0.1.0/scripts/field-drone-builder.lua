-- scripts/field-drone-builder.lua
-- Nanobots-style scripted builder for the Field Drone Depot.
--
-- Each tick-call: for every tracked depot, find ghosts within construction
-- radius, pull items from nearby Field Chests (and player inventory after
-- final research), consume a Field Charge from the depot's material slots,
-- revive the ghost, and fire a cosmetic projectile.
--
-- No real construction-robot entities are involved.

local builder = {}

local DEPOT_NAME         = "pb-field-drone-depot"
local CHEST_NAME         = "pb-field-chest"
local PROJECTILE_NAME    = "pb-field-drone-projectile"
local AMMO_NAME          = "pb-field-charge"
local PLAYER_SOURCE_TECH = "pb-field-depot-capacity-4"

local BUILD_PER_TICK = settings.startup["pb-field-depot-build-per-tick"].value

-- ── Depot tracking ──────────────────────────────────────────────────────────

function builder.init()
  storage.field_depots = storage.field_depots or {}
  storage.field_player_source = storage.field_player_source or {}

  for _, surface in pairs(game.surfaces) do
    local depots = surface.find_entities_filtered({ name = DEPOT_NAME })
    for _, depot in pairs(depots) do
      storage.field_depots[depot.unit_number] = depot
    end
  end

  for _, force in pairs(game.forces) do
    local tech = force.technologies[PLAYER_SOURCE_TECH]
    storage.field_player_source[force.index] = tech and tech.researched or false
  end
end

function builder.on_built(event)
  local entity = event.entity
  if not (entity and entity.valid) then return end
  if entity.name == DEPOT_NAME then
    storage.field_depots[entity.unit_number] = entity
  end
end

function builder.on_removed(event)
  local entity = event.entity
  if not (entity and entity.valid) then return end
  if entity.name == DEPOT_NAME then
    storage.field_depots[entity.unit_number] = nil
  end
end

function builder.on_research_finished(event)
  local tech = event.research
  if tech.name == PLAYER_SOURCE_TECH then
    storage.field_player_source[tech.force.index] = true
  end
end

-- ── Item helpers ─────────────────────────────────────────────────────────────

local function depot_has_ammo(depot)
  local inv = depot.get_inventory(defines.inventory.roboport_material)
  if not inv then return false end
  return inv.get_item_count(AMMO_NAME) > 0
end

local function consume_ammo(depot)
  local inv = depot.get_inventory(defines.inventory.roboport_material)
  inv.remove({ name = AMMO_NAME, count = 1 })
end

local function inventory_has_items(inv, items)
  for _, item_stack in pairs(items) do
    if inv.get_item_count(item_stack.name) < item_stack.count then
      return false
    end
  end
  return true
end

local function consume_items_from_inventory(inv, items)
  for _, item_stack in pairs(items) do
    inv.remove({ name = item_stack.name, count = item_stack.count })
  end
end

local function find_chest_with_items(chests, items)
  for _, chest in pairs(chests) do
    if chest.valid then
      local inv = chest.get_inventory(defines.inventory.chest)
      if inv and inventory_has_items(inv, items) then
        return chest
      end
    end
  end
  return nil
end

local function find_player_with_items(characters, items)
  for _, character in pairs(characters) do
    if character.valid then
      local player = character.player
      if player then
        local inv = player.get_inventory(defines.inventory.character_main)
        if inv and inventory_has_items(inv, items) then
          return player
        end
      end
    end
  end
  return nil
end

local function spawn_projectile(surface, source_pos, target_pos)
  surface.create_entity({
    name     = PROJECTILE_NAME,
    position = source_pos,
    target   = target_pos,
    speed    = 0.3,
  })
end

-- ── Alert helper ────────────────────────────────────────────────────────────

local function raise_no_chest_alert(depot)
  for _, player in pairs(depot.force.players) do
    player.add_custom_alert(
      depot,
      { type = "item", name = CHEST_NAME },
      { "pb-msg.depot-needs-chest" },
      true
    )
  end
end

-- ── Core build loop ─────────────────────────────────────────────────────────

function builder.on_tick()
  if not storage.field_depots then return end

  for unit_number, depot in pairs(storage.field_depots) do
    if not (depot and depot.valid) then
      storage.field_depots[unit_number] = nil
      goto next_depot
    end

    if not depot_has_ammo(depot) then goto next_depot end

    local surface = depot.surface
    local pos     = depot.position
    local radius  = depot.prototype.construction_radius
    local force   = depot.force

    local chests = surface.find_entities_filtered({
      name     = CHEST_NAME,
      position = pos,
      radius   = radius,
    })

    local player_source = storage.field_player_source
                          and storage.field_player_source[force.index]
    local characters
    if player_source then
      characters = surface.find_entities_filtered({
        type     = "character",
        position = pos,
        radius   = radius,
        force    = force,
      })
      if #characters == 0 then characters = nil end
    end

    if #chests == 0 and not characters then
      raise_no_chest_alert(depot)
      goto next_depot
    end

    local ghosts = surface.find_entities_filtered({
      type     = "entity-ghost",
      position = pos,
      radius   = radius,
    })

    local tile_ghosts = surface.find_entities_filtered({
      type     = "tile-ghost",
      position = pos,
      radius   = radius,
    })

    for _, tg in pairs(tile_ghosts) do
      ghosts[#ghosts + 1] = tg
    end

    local built = 0
    for _, ghost in pairs(ghosts) do
      if built >= BUILD_PER_TICK then break end
      if not (ghost and ghost.valid) then goto next_ghost end

      local proto = ghost.ghost_prototype
      local items_needed = proto.items_to_place_this
      if not items_needed or #items_needed == 0 then goto next_ghost end

      local source_inv
      local source_entity

      local chest = find_chest_with_items(chests, items_needed)
      if chest then
        source_inv = chest.get_inventory(defines.inventory.chest)
        source_entity = chest
      elseif characters then
        local player = find_player_with_items(characters, items_needed)
        if player then
          source_inv = player.get_inventory(defines.inventory.character_main)
          source_entity = player.character
        end
      end

      if not source_inv then goto next_ghost end
      if not depot_has_ammo(depot) then break end

      consume_items_from_inventory(source_inv, items_needed)
      consume_ammo(depot)

      local ghost_pos = ghost.position
      ghost.revive({ raise_revive = true })

      spawn_projectile(surface, pos, ghost_pos)
      built = built + 1

      ::next_ghost::
    end

    ::next_depot::
  end
end

return builder
